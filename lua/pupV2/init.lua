...
function M.grep_files()
	...
	local command = nil
	if M.file_hide_allowed then
		command = string.format('rg --files --hidden')
	else
		command = string.format('rg --files --ignore-file %s %s', grep_config, path)
	end
...
local function find_files_promise()
	return function(resolve, reject)
		...
		local param4 = nil
		if M.file_hide_allowed then
			param0 = string.format('rg')
			param1 = string.format('--files')
			param2 = string.format('--hidden')
		else
			-- command = string.format('rg --files --ignore-file %s %s', grep_config, path)
			param0 = string.format('rg')
			param1 = string.format('--files')
			param2 = string.format('--ignore-file')
			param3 = grep_config
			param4 = path
		end

		-- local job = vim.system({ 'rg', '--files', '--ignore-file', grep_config, path }, {
		local job = vim.system({ param0, param1, param2, param3, param4 }, {
			text = true
		}, function(result)
			vim.schedule(function()
...
function M.pick_files_system()
	...
	while true do
		...
		elseif char_str == '#' then
			...
			--   selected_index = math.max(1, selected_index - 1)
			--   update_display()
		elseif char_str == '*' then -- Ctrl+n (shift out)
			M.file_hide_allowed = not M.file_hide_allowed
			update_display()
